...
namespace http
{
    ...
    Test(config, ip_not_string)
    {
        try
        {
            http::parse_configuration(
                "tests/configs/config_ip_not_string.json");
            cr_assert(false);
        }
        catch (const http::InitializationError &e)
        {
            cr_assert(strcmp(e.what(), "Initialization error: invalid ip")
                      == 0);
        }
    }
    ...
    Test(config, port_not_int)
    {
        ...
        {
            cr_assert(strcmp(e.what(), "Initialization error: invalid port")
                      == 0);
        }
    }
    ...
    Test(config, auth_basic_error)
    {
        ...
        {
            http::parse_configuration(
                "tests/configs/config_auth_basic_error.json");
            cr_assert(false);
        }
        catch (const http::InitializationError &e)
        {
    ...
    Test(config, auth_basic_not_string)
    {
        ...
        {
            http::parse_configuration(
                "tests/configs/config_auth_not_string.json");
            cr_assert(false);
        }
    ...
    Test(config, auth_basic_empty)
    {
        ...
        {
            cr_assert(strcmp(e.what(), "Initialization error: Empty auth")
                      == 0);
        }
    }
    ...
    Test(config, ssl_key_error)
    {
        ...
        {
            http::parse_configuration(
                "tests/configs/config_ssl_key_error.json");
            cr_assert(false);
        }
        catch (const http::InitializationError &e)
        {
    ...
    Test(config, ssl_cert_error)
    {
        ...
        {
            http::parse_configuration(
                "tests/configs/config_ssl_cert_error.json");
            cr_assert(false);
        }
        catch (const http::InitializationError &e)
        {
    ...
    Test(config, multiple_default_vhosts)
    {
        ...
        {
            cr_assert(strcmp(e.what(),
                             "Initialization error: Default vhost must be "
                             "unique and valid")
                      == 0);
        }
    }
    ...
    Test(config, reverse_proxy)
    {
        ...
        cr_assert(server_config.upstreams.size() == 1);
        cr_assert(server_config.upstreams.count("backend") == 1);
        cr_assert(server_config.upstreams["backend"].method
                  == http::UpstreamConfig::Method::FAIL_ROBIN);

        cr_assert(server_config.upstreams["backend"].hosts.size() == 2);
        cr_assert(server_config.upstreams["backend"].hosts[0].ip
                  == "127.0.0.1");
        cr_assert(server_config.upstreams["backend"].hosts[0].port == 5000);
        cr_assert(server_config.upstreams["backend"].hosts[0].health == "/vie");
        cr_assert(server_config.upstreams["backend"].hosts[0].weight == 15);

        cr_assert(server_config.vhosts.size() == 2);
        auto vhost = server_config.vhosts[0];
        ...
        auto vhost2 = server_config.vhosts[1];
